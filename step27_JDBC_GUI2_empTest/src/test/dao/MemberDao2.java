...
import java.util.List;
	...
	public class MemberDao2 {
		...
		// DB 에 회원정보를 저장하는 메소드
//		public void insert(int num,String name, String addr) {
//		 이런식으로 정의하게되면 전달받는 인자가 10개쯤 되면 나열하는것도 힘드므로 MemberDto 로 전달받음	
//		}
		//retrun 받기 위해서 값이 잘 전달되었는지 확인하기 위해서 boolean형으로 받음
		...
		public MemberDto getData(int num) {
			...
			try{
				...
				rs=pstmt.executeQuery();
				//반복문 돌면서 cursor 를 한칸씩 내린다. 
				if(rs.next()){//커서를 한칸 내려서 커서가 위치한 곳에서 회원정보를 얻어온다.
				//num은 값을넘겨주므로 필요가 없다 ~ 알고있으니까 ~
					String name=rs.getString("name");
					String addr=rs.getString("addr");
					//MemberDto 객체에 담는다
					dto=new MemberDto(num,name,addr);
//					dto.setNum(num); 선언과 동시에 쑉 넣어버림
//					dto.setName(name);
//					dto.setAddr(addr);
